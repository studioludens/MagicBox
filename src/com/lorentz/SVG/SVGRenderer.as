	...
	public class SVGRenderer extends Sprite{		
		...
		private function visitText(elt:Object):Sprite {
			...
			for each(var childElt:Object in elt.children) {
				...
				// is the font available? then set embedding to true
				// this is the only time you can show a font with rotation and alpha
				
				if( SVGUtil.isFontAvailable(dTFormat.font) ){
					// use the embedded font
					tField.embedFonts = true;	// EDIT: Alex, allow for rotation of text
				}
				
				// EDIT ALEX: add opacity
				tField.alpha = elt.finalStyle["opacity"] ? elt.finalStyle["opacity"] : 1;
